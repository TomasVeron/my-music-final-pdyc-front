...
const ModalDeletePlaylist = ({ visible, closeHandler,setPlaylists,  playlists, playlist, user }) => {
  let onSubmit = async (data) => {
    axiosInstance.defaults.headers.common["Authorization"] = "Bearer " + user?.stsTokenManager?.accessToken;
    await axiosInstance
      .delete(`/ms-playlists/playlists/${playlist.id}`, {})
      .then((res) => {
        closeHandler();
      })
      .catch((error) => {
          ...
          <form onSubmit={handleSubmit} className="bg-white py-2 px-4 rounded shadow-lg">
            ...
            <Modal.Body>
              <Container display="flex" direction="column" justify="center" alignItems="center" fluid>
                <Text b>Are you sure you want to delete the playlist ?</Text>
                <Spacer y={1} />
              </Container>
            </Modal.Body>
            <Modal.Footer>
              ...
              <Button auto flat type="submit" color="error" ms onClick={closeHandler}>
                Delete
              </Button>
            </Modal.Footer>
          </form>
